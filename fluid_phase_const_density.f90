program fluid_phase_const_density
  ...
  integer(i4b), parameter :: N=500, N1=26
  integer(i4b)::i,i3,j3,j,ndiv,i5,j5,i2,seedsize,i4,err,icount
  integer,allocatable:: seed(:)
  real(wp) :: position(3,N), momentum(3,N), zeta, dt, Q, g, instant_kbT,kbT0, t, L, av_momentum(3),rho,c,V,&
  & position_Ein(3,N), theta(4), spring_alpha, square_displacement_ave(N), displacement(3,N),delta_r,gr_dash(100),&
  & lambda1,lambda2,x(N1),w(N1),f(N1), lambda, free_energy, free_energy_, spring_alpha_array(N), pressure,&
  ...
  open(11,file='data_kbT3.0_again.dat')
  open(12,file='position_kbT3.0_again.dat')
  open(13,file='momentum_kbT3.0_again.dat')
  open(14,file='position_Ein.dat')
  open(15,file='RDF_kbT3.0_again.dat')
  open(16,file='spring_alpha.dat')
  open(17,file='data2_kbT3.0_again.dat')
  open(18,file='data_free_energy.dat')
  open(19,file='data3.dat')
  open(20,file='RDF_first_kbT3.0_again.dat')
  open(21,file='force.dat')
  open(22,file='force_first.dat')
  open(23,file='paramater.dat')
  open(24,file='DLVO_potential.dat')
  open(25,file='integrand_kbT3.0_again.dat')
  open(26,file='data_DLVO_ptential_maxmin.dat')
  open(27,file='data_DLVO_force_max.dat')
  open(28,file='position_first_kbT3.0_again.dat')
  ...
  enddo
  instant_kbT = (1/g)*(sum(momentum0**2))
  momentum = momentum0
  ! c=(0.5/kbT)**0.5
  ...
  do i =0,25000
    ...
    if(mod(i,250)==0)then
      instant_kbT = (1/g)*(sum(momentum**2))
      write (11,'(5g18.8)') t, instant_kbT,V,total_potential(theta,param,N,position,L),instantaneous_P(theta,param,N,L,V,position,momentum)
    endif
  ...
  do i4 = 1,N1
    ...
    do i =0,50000
      ...
      if(mod(i,500)==0)then
        instant_kbT = (1/g)*(sum(momentum**2))
        write (17,'(6g18.8)') t, instant_kbT,V,L,total_potential(theta,param,N,position,L),instantaneous_P(theta,param,N,L,V,position,momentum)
      endif
    ...
    write(25,'(5g18.8)') kbT0, potential_ave, potential_ave/V, pressure_ave, i
